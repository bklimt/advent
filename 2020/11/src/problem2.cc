...
bool IsOccupied(const vector<string> &input, int i, int j, int di, int dj) {
  i += di;
  j += dj;
  ...
  if (input[i][j] == '.') {
    return IsOccupied(input, i, j, di, dj);
  }
  return input[i][j] == '#';
}
  ...
  *seats = 0;
  ...
  for (int i = 0; i < input.size(); i++) {
    ...
    for (int j = 0; j < input[i].size(); j++) {
      ...
      if (c != '.') {
        int people_around = 0;
        if (IsOccupied(input, i, j, -1, -1)) { people_around++; }
        if (IsOccupied(input, i, j, -1,  0)) { people_around++; }
        if (IsOccupied(input, i, j, -1,  1)) { people_around++; }
        if (IsOccupied(input, i, j,  0, -1)) { people_around++; }
        if (IsOccupied(input, i, j,  0,  1)) { people_around++; }
        if (IsOccupied(input, i, j,  1, -1)) { people_around++; }
        if (IsOccupied(input, i, j,  1,  0)) { people_around++; }
        if (IsOccupied(input, i, j,  1,  1)) { people_around++; }
